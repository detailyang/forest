...
pub static ADDRESS: &actorv5::STORAGE_POWER_ACTOR_ADDR = &actorv5::STORAGE_POWER_ACTOR_ADDR;

/// Power actor method.
/// TODO: Select based on actor version
pub type Method = actorv5::power::Method;
    ...
    pub fn list_all_miners<BS: BlockStore>(&self, s: &BS) -> Result<Vec<Address>, Box<dyn Error>> {
        match self {
            ...
            State::V4(st) => {
                ...
                let mut miners = Vec::new();
                claims.for_each(|k, _: &actorv4::power::Claim| {
                    miners.push(Address::from_bytes(&k.0)?);
                    Ok(())
                })?;
            ...
            State::V5(st) => {
                ...
                let mut miners = Vec::new();
                claims.for_each(|k, _: &actorv5::power::Claim| {
                    miners.push(Address::from_bytes(&k.0)?);
                    Ok(())
                })?;
